...
OpenClaw 官方仓库、官网和文档都把它描述为运行在个人设备上的 AI 助手系统，而不是托管式 SaaS 聊天产品。[官网](https://openclaw.ai/)、[官方仓库](https://github.com/openclaw/openclaw)、[官方文档](https://docs.openclaw.ai/)

其核心思想可以概括为三点：

- `self-hosted gateway`：真正长期运行的是 Gateway，assistant 只是建立在 Gateway 之上的产品体验。
    
- `multi-surface access`：它可以接入 WhatsApp、Telegram、Discord、Slack、iMessage、Web UI、CLI 等多种入口。
    
- `tool-using assistant`：它不是只负责回答问题，而是把浏览器、终端、文件、自动化等能力挂到同一个 agent 体系里。
    

官方 `VISION.md` 里对方向的表述很明确：目标不是做另一个只会对话的 AI，而是做“the AI that actually does things”。[VISION.md](https://raw.githubusercontent.com/openclaw/openclaw/main/VISION.md)
...
![fig2](/assets/img/posts/openclaw/papers/2603.10165/key_figures/fig2.png)
...
![fig1](/assets/img/posts/openclaw/papers/2602.08234/key_figures/fig1.png)
...
![fig1](/assets/img/posts/openclaw/papers/2603.17187/key_figures/fig1.png)
...
评估"Agent"，应该是把模型和Harness一起评估，同一模型在不同harness上差距很大。
...
# 附录：Hermes Agent —— 与 OpenClaw 的异同

> [Hermes Agent](https://hermes-agent.nousresearch.com/docs/) 是 NousResearch 开源的另一套自托管 AI 助手系统，定位与 OpenClaw 高度重叠，但在架构哲学和工程取舍上走了一条截然不同的路。本节基于源码，梳理几个最值得关注的差异点。

两者解决的是同一个问题：**如何让 LLM 成为一个真正长期运行、跨平台可用的个人助手**。两者都采用了 Gateway 长期驻留 + 多消息平台接入 + 工具调用 + 持久化记忆的基本架构，也都支持 Telegram、Discord、Slack 等主流平台。

**不过Hermes Agent是Python实现的，OpenClaw是Typescript实现的。**

## Hermes 与 OpenClaw的一些差异

### 1. Agent 缓存策略：Hermes 对 prefix cache 的优化

两者在进程架构上其实是一样的——Gateway 和 Agent 都跑在同一个进程内，都通过直接函数调用驱动 Agent 执行。OpenClaw 用 `runEmbeddedPiAgent`（"Embedded"这个名字本身就说明了一切），Hermes 用 `from run_agent import AIAgent` 直接 import 并实例化。

但在这个共同基础上，两者对**消息状态复用**的设计取向出现了明显分歧。

OpenClaw 没有跨消息的 Agent 实例缓存。每次 `agentCommand` 调用都会新建一个 `contextEngine` 实例，从 `sessionFile` 重新加载历史消息，system prompt 也重新构建。它的持久化依赖磁盘：会话历史写入 `.jsonl` 文件，Skills 快照存在 `sessionEntry` 里避免重复扫描 workspace，但 Agent 对象本身不跨 turn 保留在内存里。

Hermes 则把 Agent 实例缓存作为核心设计：`GatewayRunner` 持有一个 `_agent_cache: Dict[str, tuple]`，按 `session_key` 缓存 `AIAgent` 实例，config 签名（model、provider、toolset 列表、ephemeral prompt 的 SHA256）不变时直接复用同一个对象。这个设计的核心动机是**保住 provider prefix cache**——如果每条消息都新建一个 AIAgent，system prompt 就会被重新构建，Anthropic 等提供商的 prompt cache 就会失效，成本可能上涨 10 倍。Hermes 源码注释里写得很直白：

> *Without this, a new AIAgent is created per message, rebuilding the system prompt (including memory) every turn — breaking prefix cache and costing ~10x more on providers with prompt caching (Anthropic).*
...
```plaintext
你发第1条消息 → 新建 contextEngine → 从磁盘 .jsonl 加载历史 → 构建 system prompt → 调用 API → contextEngine.dispose() 
你发第2条消息 → 新建 contextEngine → 从磁盘 .jsonl 加载历史 → 构建 system prompt → 调用 API → contextEngine.dispose() 
你发第3条消息 → 新建 contextEngine → 从磁盘 .jsonl 加载历史 → 构建 system prompt → 调用 API → contextEngine.dispose()
```
同一个 session，每条消息都走一遍这个流程。历史消息是从磁盘文件读的，不是从内存里取的。
>`userTime`会变，Anthropic 的 prefix cache 要求 system prompt 的**字节完全一致**才能命中。只要 `userTime` 变了（哪怕只是分钟数变了），整个 system prompt 的字节就不同，**prefix cache 必然 miss**。

**Hermes 的情况：**
```PlainText
你发第1条消息 → 新建 AIAgent → 构建 system prompt → 调用 API → AIAgent 留在 _agent_cache 里 
你发第2条消息 → 从 _agent_cache 取出同一个 AIAgent → system prompt 没变 → 调用 API（prefix cache 命中） 
你发第3条消息 → 从 _agent_cache 取出同一个 AIAgent → system prompt 没变 → 调用 API（prefix cache 命中）
```

##### 换 session 的情况

两者都会重新构建——这是理所当然的，新 session 就是新的上下文。Hermes 的 `_agent_cache` 是按 `session_key` 存的，换 session 就是换 key，自然取不到缓存。

OpenClaw 的设计哲学是"**无状态服务 + 磁盘持久化**"——每次请求独立处理，状态全部落盘，好处是容错性强（进程崩溃不丢数据），也更适合未来分布式部署。代价是每次都有从磁盘加载的 I/O 开销，以及 system prompt 无法命中 provider 的 prefix cache。

### 2. 记忆系统：检索型 vs 建模型

两者的记忆系统在表面上都有本地 Markdown 文件，但整体结构也有一些区别。

| 维度 | OpenClaw | Hermes |
|---|---|---|
| **本地记忆文件** | `MEMORY.md` + 日志型 session 文件 | `MEMORY.md` + `USER.md`（冻结快照注入） |
| **检索方式** | 向量嵌入 + SQLite FTS5 语义搜索 | 文件直接注入 system prompt |
| **嵌入后端** | Ollama / OpenAI / Voyage / Gemini 可选 | 无（不做向量检索） |
| **云端用户建模** | ❌ 无 | ✅ Honcho（可选，需 API key） |
| **跨 session 持久性** | 本地磁盘 | 本地磁盘 + Honcho 云端持续演化 |
| **Prefix cache 保护** | ❌ 每次重建，时间戳变化必然 miss | ✅ 冻结快照，session 内 system prompt 稳定 |
| **用户建模深度** | 无主动建模 | Dialectic LLM 推理，`user_peer` / `ai_peer` 双身份 |
**Hermes 的记忆架构**是双层设计：

- **第一层：本地文件快照**。`MEMORY.md`（Agent 个人笔记）和 `USER.md`（用户画像）在 session 启动时由 `MemoryStore.load_from_disk()` 读取，并立即做一次**冻结快照**（`_system_prompt_snapshot`）。session 内的写操作（`memory` 工具的 add/replace/remove）只更新磁盘，**不改变 system prompt**——这是刻意的设计，目的是保住 prefix cache 的稳定性。

- **第二层：Honcho 云端用户建模**。[Honcho](https://github.com/plastic-labs/honcho) 是 Plastic Labs 开发的云端服务，Hermes 把它作为可选的第三层记忆后端接入（需配置 API key 或自托管）。与本地文件的"存档"思路不同，Honcho 维护的是持续演化的**结构化用户画像**——它同时建模 `user_peer` 和 `ai_peer` 两个身份，通过 `dialectic_query()` 在 Honcho 后端跑 LLM 推理，回答"关于这个用户我知道什么"，再把结论注入 system prompt。为了不阻塞响应，Hermes 用 `prefetch_dialectic()` 在后台线程提前跑推理，下一轮直接 `pop_dialectic_result()` 取结果。

两者最根本的差异在于**记忆的粒度和持久性**：OpenClaw 的记忆是"检索相关片段"，Hermes 的 Honcho 是"持续推理用户是谁"。前者适合快速找到过去的具体信息，后者适合跨 session 积累对用户的深层理解。

### 3. 后台自我改进循环

这是 Hermes 一个相当有意思的设计，OpenClaw 没有对应机制。

Hermes 的 `run_agent.py` 里有一个 `_spawn_background_review` 方法：每次主 agent 把回复发出去之后，会在后台 `threading.Thread` 里 spawn 一个完整的 `AIAgent` fork，拿着当前对话的快照跑一轮推理，专门决定"这次对话里有什么值得记住的，有什么应该变成可复用的 skill"，然后直接写入共享的 `MemoryStore`。整个过程对用户完全透明——stdout/stderr 全部重定向到 `/dev/null`，只有真正写入成功时才会发一条 `💾 Memory updated` 的通知。

```
用户消息 → 主 Agent 回复 → 回复发出
                              ↓ (后台线程，不阻塞)
                         review_agent fork
                              ↓
                    决定写 memory / 创建 skill
...
触发条件分两种：`review_memory`（决定是否更新记忆）和 `review_skills`（决定是否创建新 skill），两者同时触发时用 `_COMBINED_REVIEW_PROMPT`，最多跑 8 轮迭代。review agent 用完即销毁，不留任何状态。

OpenClaw 的记忆写入是 Agent 在主对话流程中**主动调用 `memory` 工具**完成的——需要 Agent 自己判断"我应该记住这个"，然后显式调用。Hermes 的后台 review 则是把这个判断**外包给一个独立的 Agent**，在回复结束后异步完成，不占用主对话的 context window，也不影响响应延迟。

### 4. RL 研究基础设施

这是两者差距最大的一个维度。

Hermes 的 `environments/` 目录是一套完整的 RL 训练基础设施，直接对接 [Atropos](https://github.com/NousResearch/atropos) 框架。继承体系是三层：`BaseEnv`（Atropos 基类）→ `HermesAgentBaseEnv`（注入 Hermes agent loop、toolset 分发、ScoredDataGroup 构建等公共逻辑）→ 具体环境（只需实现 `setup` / `get_next_item` / `format_prompt` / `compute_reward` 四个方法）。目前已内置了 SWE-bench（`hermes_swe_env`）、TerminalBench 2、YC Bench、TBLite 等多个 benchmark 环境。

数据管道也是完整的：`batch_runner.py` 支持多进程并行跑 agent rollout，带断点续跑和 checkpoint；`trajectory_compressor.py` 负责把长轨迹压缩到 token 预算内（保护首尾 turn，压缩中间部分），输出可直接用于训练的 JSONL 格式。此外还有专门的 `rl_cli.py`，把 RL 工作流的 system prompt、超时策略、工具集都单独配置好，启动即用。

OpenClaw 本身没有这些。OpenClaw-RL、Claw-R1、MetaClaw 这些工作都是社区和研究者**在 OpenClaw 之外**搭建的训练流程，OpenClaw 的代码仓库里找不到任何 RL 环境或训练数据管道的代码。这不是缺陷，而是定位不同——OpenClaw 是一个面向用户的产品，Hermes 是一个面向研究者的实验平台，RL 训练基础设施是它的核心竞争力之一。
...
| 维度           | OpenClaw                           | Hermes                                        |
| ------------ | ---------------------------------- | --------------------------------------------- |
| **Agent 缓存** | 无跨消息缓存，每次重新组装 prompt               | `_agent_cache` 跨 turn 复用，保住 prefix cache      |
| **记忆层次**     | 本地向量检索（SQLite + 嵌入），无云端            | 本地冻结快照 + Honcho 云端用户建模                        |
| **后台自我改进**   | ❌ 无，Agent 主动调用 memory 工具写入         | ✅ 回复后 spawn review agent，异步决策写 memory / skill |
| **RL 训练**    | 社区外部工作（OpenClaw-RL、Claw-R1）        | 内置 environments/ + Atropos 接口，第一公民            |
| **扩展方式**     | 插件系统（npm 包，声明 openclaw.extensions） | Skills（Markdown 文件）+ 工具自注册                    |
| **定位**       | 产品优先，面向普通用户                        | 研究优先，面向 AI 研究者和开发者                            |
| **社区生态**     | ClawHub、Moltbook、5700+ Skills      | 相对早期，NousResearch 主导                          |

> **一句话总结**
> OpenClaw 是**把 AI 助手做成产品**的路子——插件生态、多平台接入、用户友好的配置体验是核心竞争力；Hermes 是**把 AI 助手做成研究平台**的路子——Agent 实例缓存、Honcho 用户建模、内置 RL 训练基础设施是它真正独特的地方。两者并不互斥，Hermes 的 prefix cache 策略和 Honcho 用户建模思路值得 OpenClaw 生态借鉴。
{: .block-warning }
