...
从公开时间线看，openclaw的演化过程，可以把它概括成四个阶段。[^openclaw-lore]
...
OpenClaw 并不是一个套在 AI 模型 API 外层的聊天机器人封装，而是一个**面向 AI Agent 的操作系统**。[^openclaw-architecture-explained]
...
OpenClaw 的中心不是某个聊天窗口，而是一个长期运行的 Gateway。Gateway 负责承接所有消息面、控制端和节点设备连接；CLI、Web UI、桌面端以及 automations 都通过 WebSocket 连接到它。[^openclaw-architecture-doc]
...
OpenClaw 官方把能力层拆成三部分。[^openclaw-tools-and-plugins]
...
图：OpenClaw 插件扩展关系示意。来源见[^openclaw-architecture-explained]。
{: .figure-note }
...
图：Claude Code 的 agentic loop。来源与原图见[^claude-code-loop-diagram]。
{: .figure-note }

这里其实和claude code和codex是类似的，只不过openclaw用的是 **Pi-Agent**。[^pi-minimal-agent]
...
在 OpenClaw 里，**System Prompt 不是一段固定的"前言"**，而是 runtime 在每次 agent run 开始前动态组装出来的一份运行时说明书。[^system-prompt-doc]
...
**Context** 是每次 run 实际送给模型的全部内容，受模型 context window（token 上限）约束。它不等于 Memory——Memory 存在磁盘上可以反复加载，Context 是当前窗口里的内容，包括 system prompt、会话历史、工具调用结果和附件。可以用 `/context list` 和 `/context detail` 查看各部分的实际 token 占用，用 `/compact` 手动压缩历史腾出空间。[^context-doc]

![openclaw context arch](/assets/img/posts/openclaw/openclaw-context-arch.svg)

图：Workspace、Context Window 与 Context Engine 的关系。来源见[^context-doc]。
{: .figure-note }

**Context Engine** 控制 context 的组装和压缩，有四个生命周期钩子：Ingest（存储新消息）→ Assemble（组装送给模型的 messages）→ Compact（窗口满时压缩）→ After Turn（run 结束后收尾）。默认使用内置的 `legacy` 引擎（直通 + 委托内置压缩）；也可以通过 `plugins.slots.contextEngine` 切换为插件引擎，实现自定义的检索增强、向量召回等策略。Memory 插件和 Context Engine 是独立的：Memory 插件提供检索能力，Context Engine 决定模型最终看到什么。[^context-engine-doc]
...
Session 管理相关机制可参考[^session-docs]。
...
本节覆盖 OpenClaw 消息管道的四个核心机制。[^messages-docs]
...
图：Agent Skills 的整体工作位置。Agent 在处理任务时可发现并调用 skill，skill 再去编排本地工具、外部资源与执行环境。来源与原图见[^agent-skills-overview]。
{: .figure-note }
...
图：Claude Code Hooks 生命周期（`PreToolUse` -> 工具执行 -> `PostToolUse`）。来源与原图见[^claude-hooks-lifecycle]。
{: .figure-note }

![claude code hook resolution](/assets/img/posts/openclaw/claude-code-hook-resolution.svg)

图：Claude Code hook resolution，全局、本地与工具级别的优先级。来源与原图见[^claude-hooks-resolution]。
{: .figure-note }
...
**ClawHub** 是 OpenClaw 的官方技能与插件市场，随 v2026.3.22 版本正式上线，定位类似 App Store，但面向 AI Agent 能力扩展。官方文档见[^clawhub-doc]。
...
**Moltbook** 是基于 OpenClaw 构建的 **AI Agent 专属社交平台**，由 Matt Schlicht 和 Ben Parr 于 2026 年 1 月底创立。定位类似 Reddit，但规则只有一条：**只有 AI Agent 可以发帖，人类只能围观**。官网与报道见[^moltbook-links]。
...
**InStreet**（Instance Street）是字节跳动旗下 **Coze 团队**于 2026 年 3 月 9 日推出的中文 AI Agent 社区平台，基于 Coze Code 构建，目前处于 Beta 阶段。[^instreet-site]
...
图：OpenSpace 整体框架。技能从执行中被捕获、进化，并通过云端社区在 agent 之间流通。来源见[^openspace]。
{: .figure-note }
...
Deepmind 的 Staff Engineer Philipp Schmid 甚至直接给出了一个判断：“The Harness is the Dataset. Competitive advantage is now the trajectories your harness captures（Harness 本身就是数据集。现在真正的竞争优势，在于你的 harness 能捕获到怎样的执行轨迹）。”[^harness-new-dataset]
...
## 7.2 趋势二：从模型优化走向 Model-Harness Co-Training
...
[^openclaw-architecture-explained]: [OpenClaw Architecture, Explained](https://www.linkedin.com/pulse/openclaw-architecture-explained-paolo-perazzo-g4ubc/)
[^openclaw-architecture-doc]: [OpenClaw 架构文档](https://docs.openclaw.ai/concepts/architecture)
[^openclaw-lore]: [Lore 页面](https://docs.openclaw.ai/zh-CN/start/lore)
[^openclaw-tools-and-plugins]: [Tools and Plugins](https://docs.openclaw.ai/tools)
[^system-prompt-doc]: [System Prompt](https://docs.openclaw.ai/concepts/system-prompt)
[^context-doc]: [Context](https://docs.openclaw.ai/concepts/context)
[^context-engine-doc]: [Context Engine](https://docs.openclaw.ai/concepts/context-engine)
[^session-docs]: [Session](https://docs.openclaw.ai/concepts/session)；[Session Pruning](https://docs.openclaw.ai/concepts/session-pruning)；[Session Tools](https://docs.openclaw.ai/concepts/session-tool)
[^messages-docs]: [Messages](https://docs.openclaw.ai/concepts/messages)；[Streaming](https://docs.openclaw.ai/concepts/streaming)；[Queue](https://docs.openclaw.ai/concepts/queue)；[Retry](https://docs.openclaw.ai/concepts/retry)
[^claude-code-loop-diagram]: [How Claude Code works](https://code.claude.com/docs/en/how-claude-code-works)
[^pi-minimal-agent]: [Pi: The Minimal Agent Within OpenClaw](https://lucumr.pocoo.org/2026/1/31/pi/)
[^agent-skills-overview]: [Agent Skills overview](https://platform.claude.com/docs/en/agents-and-tools/agent-skills/overview)
[^claude-hooks-lifecycle]: [Hooks reference](https://code.claude.com/docs/en/hooks)
[^claude-hooks-resolution]: [Hooks reference](https://code.claude.com/docs/en/hooks)
[^clawhub-doc]: [ClawHub 官方文档](https://docs.openclaw.ai/tools/clawhub)
[^moltbook-links]: [moltbook.com](https://www.moltbook.com/)；[TechCrunch](https://techcrunch.com/2026/03/10/meta-acquired-moltbook-the-ai-agent-social-network-that-went-viral-because-of-fake-posts/)
[^instreet-site]: [InStreet 官网](https://instreet.coze.site/)
[^openspace]: [OpenSpace](https://github.com/HKUDS/OpenSpace)
[^harness-new-dataset]: [Harness is the New Dataset：模型智能提升的下一个关键方向](https://mp.weixin.qq.com/s/9qI83Ne-Ac_R9y-yJ6SVnQ)
